...
namespace DI
{
    public class DIContainer
    {
        ...
        public T Resolve<T>(string tag = null)
        {
            ...
            {
                throw new Exception(
                    $"Cyrcle dependency for tag {key.tag} and type {key.Item2.FullName}");
            }
            ...
            throw new Exception(
                $"Couldn`t find tag{tag} and type {key.Item2.FullName}");
        }
        private void Register<T>((string, Type) key, Func<DIContainer, T> factory, bool isSingleton)
        {
            ...
            _registrations[key] = new DIRegistration
            {
                Factory = c => factory(c), //Factory = c:DIContainer => factory,
